...
6. O PostHog já está em produção hoje. Isso não estava no escopo original do prompt, que falava só de GA4, GTM e Pixel. A usuária não soube responder de imediato e pediu para eu explicar o que está em jogo, ainda sem decisão registrada. O que muda: se a Etapa 1 cobrir só o GA4, a sanitização de URL (remover parâmetros que não sejam UTM antes de qualquer envio) vale só para o gtag/GTM, e o PostHog continua enviando a URL completa como já faz hoje, incluindo qualquer parâmetro de busca presente. Se cobrir os dois, a mesma função de sanitização passa a valer também para o `$pageview` do PostHog e para os eventos de servidor em `lib/posthog.ts`, o que é mais seguro e consistente com a regra de privacidade deste projeto, mas é código a mais e precisa ser tocado com cuidado por já estar em produção hoje, ao contrário do GA4 que ainda nem está coletando.
