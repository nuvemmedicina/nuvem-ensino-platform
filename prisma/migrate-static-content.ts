/**
 * One-time migration: populates course DB fields (objectives, targetAudience,
 * includes, startDateLabel) from the static content map previously hardcoded
 * in the public course page.
 *
 * Run with: npx tsx prisma/migrate-static-content.ts
 */
...
async function main() {
  console.log("Iniciando migração de conteúdo estático para o banco...\n");

  for (const [slug, content] of Object.entries(staticContent)) {
    const course = await prisma.course.findUnique({ where: { slug } });

    if (!course) {
      console.log(`⚠  Curso não encontrado no banco: ${slug}`);
      continue;
    }

    const objectives     = content.objectives.join("\n");
    const targetAudience = content.targetAudience.join("\n");
    const includes       = content.includes.join("\n");

    await prisma.course.update({
      where: { slug },
      data: {
        objectives:     course.objectives     ?? objectives,
        targetAudience: course.targetAudience ?? targetAudience,
